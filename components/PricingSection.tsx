  ...
  const handleCheckout = async (price: number) => {
    try {
      ...
      if (data?.url) {
        ...
        alert("Erro ao criar checkout. Veja o console.");
      }
    } catch (error) {
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 items-start">
          {packages.map((pkg, idx) => (
            <div
              key={idx}
              className={`relative flex flex-col h-full bg-[#0a0f1c] rounded-2xl overflow-hidden border ${
                pkg.isPopular
                  ? 'border-yellow-400 shadow-[0_0_20px_rgba(250,204,21,0.3)] scale-105 z-10'
                  : 'border-gray-800'
              ...
              <div className="p-6 flex-1 flex flex-col">
                ...
                <div className="mt-auto text-center">
                  <div className="flex items-baseline justify-center text-white mb-6">
                    ...
                    <span className="text-5xl font-black text-yellow-400 ml-1">
                      {pkg.price}
                    </span>
                  </div>

                  <button
                    onClick={() => handleCheckout(pkg.price)}
                    className="w-full py-4 rounded-full font-bold uppercase tracking-wide transition-colors bg-green-500 hover:bg-green-600 text-white"
                  >
                    Quero o {pkg.name}
                  </button>
